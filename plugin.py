...
class BasePlugin:
        ...
        def getPlantData(self, idx):
                ...
                normal='0'
                #NewSvalue='%s;%s;%s'   % (val_moist, str(value) , SplitData[2])
                NewSvalue='%s;%s'       % (val_moist, normal)
                Domoticz.Log("NewSvalue = " + NewSvalue)
                Devices[sensorNumber1].Update(nValue=nValue, sValue=NewSvalue, BatteryLevel=val_bat) # 0=Normal
